...
def status_counts(payload: object) -> tuple[dict[str, int], int]:
...
def selected_channels(raw: str, archive: dict[str, int], pocketbase: dict[str, int]) -> list[str]:
    if raw == "all":
    ...
    selected = raw.split(",")
    if any(not CHANNEL_ID.fullmatch(item) for item in selected):
        raise VerificationError("each CHANNELS value must be a 17-20 digit Discord channel ID")
...
def verify(sqlite_db: str, pb_url: str, channels: str, timeout: float = 10.0) -> int:
    ...
    pb_counts, _ = status_counts(payload)
    with snapshot(sqlite_db) as connection:
        source_counts = archive_counts(connection)

    selected = selected_channels(channels, source_counts, pb_counts)
    source_total = 0
    pb_total = 0
    mismatch = False
    print("channel_id sqlite pocketbase result")
    for channel_id in selected:
        ...
        mismatch |= result != "OK"
        print(f"{channel_id} {expected} {actual} {result}")
