(function () {
  ...
  function App(){
    ...
    const older=async()=>{if(!cursor)return;const req=begin();setBusy(true);try{const data=await api(query({cursor}),{signal:req.signal});if(!req.current())return;let total=0;setEvents(prev=>{const merged=M.mergeSorted(prev,data.events);total=merged.length;return merged;});setCursor(data.next_cursor);setHasMore(data.has_more);setState(prev=>prev.replace(/^\d+ events/,`${total} events`));}catch(error){if(error.name==='AbortError'||!req.current())return;setState(error.message);}finally{if(req.current())setBusy(false);}};
